<?php
class Mjsi_Orderprint_Model_Order_Pdf_Items_Order_Default extends Mjsi_Orderprint_Model_Order_Pdf_Items_Abstract
{
	public function draw()
    {
        ...
        foreach (Mage::helper('core/string')->str_split($item->getName(), 32, true, true) as $key => $part) {
            $page->drawText($part, 32, $pdf->y-$shift[0], 'UTF-8');
            $shift[0] += 10;
        }
        ...
        foreach (Mage::helper('core/string')->str_split($this->getSku($item), 18) as $key => $part) {
            if ($key > 0) {
                $shift[2] += 10;
            }
		...
		//$page->drawText($item->getQty()*1, 425, $pdf->y, 'UTF-8');
		$page->drawText($item->getQtyOrdered()*1, 425, $pdf->y, 'UTF-8');
		
		$font =  $this->_setFontRegular();
		
        $row_total = $order->formatPriceTxt($item->getRowTotal());
        $page->drawText($row_total, 550-$pdf->widthForStringUsingFontSize($row_total, $font, 7), $pdf->y, 'UTF-8');
        ...
        $page->drawText($tax, 485-$pdf->widthForStringUsingFontSize($tax, $font, 7), $pdf->y, 'UTF-8');
